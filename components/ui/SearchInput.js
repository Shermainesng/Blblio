...
const SearchInput = () => {
  ...
  async function filterBooksHandler(encodedAuthor, encodedCat) {
    await axios
      .get(
        `https://www.googleapis.com/books/v1/volumes?q=${encodedCat}+inauthor:${encodedAuthor}&maxResults=16&key=AIzaSyCQucF_1d8cNR0dwoI43RpQK8E0xTCgjFs`
      )
  ...
  async function onSearch(e) {
    ...
    await axios
      .get(
        `https://www.googleapis.com/books/v1/volumes?q=${encodedSearchQuery}&maxResults=16&key=AIzaSyCQucF_1d8cNR0dwoI43RpQK8E0xTCgjFs`
      )
      .then(function (response) {
  ...
  function filterResponseData(unfilteredResponses) {
    ...
    for (let i = 0; i < unfilteredResponses.length; i++) {
      if (unfilteredResponses[i].volumeInfo.title) {
        if (
          unfilteredResponses[i].volumeInfo.title.toLowerCase() != "undefined"
        ) {
    ...
    <div className="row">
      ...
      <form className="col-12 col-sm-12 col-md-12 col-lg-6 row">
        <div className="d-flex flex-column justify-content-center justify-content-start align-items-start">
          ...
          <div className="btn btn-green mt-1" onClick={(e) => submitHandler(e)}>
            Search
          </div>
        </div>
      </form>
      ...
      <div className="container pt-4">
        <div className="row justify-content-md-center">
          {searchResults && <BookList items={searchResults} />}
        </div>
      </div>
    </div>
  );
